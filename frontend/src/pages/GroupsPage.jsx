  ...
  return <section>
    ...
    <table className='vm-table'><thead><tr><th>Name</th><th>Enabled</th><th>Members</th><th>Template perms</th><th>Actions</th></tr></thead><tbody>
      {rows.map(g=><tr key={g.id}><td>{g.name}</td><td>{String(g.enabled)}</td><td>{g.member_count}</td><td>{g.template_permission_count}</td><td><div className='group'>
        ...
        <button onClick={async()=>{setMembers(p=>({...p,[g.id]:await listGroupMembers(g.id)}))}}>Members</button>
        <button onClick={async()=>{const p=await listGroupTemplatePermissions(g.id); setGroupPerms(x=>({...x,[g.id]:p.template_ids||[]}))}}>Template Permissions</button>
      ...
      {Array.isArray(members[g.id])?<div className='panel'><h4>Members</h4><div className='group'>{members[g.id].map(m=><span key={m.user_id}>{m.username||m.user_id} <button onClick={async()=>{await removeGroupMember(g.id,m.user_id); setMembers(p=>({...p,[g.id]:await listGroupMembers(g.id)}))}}>x</button></span>)}</div><select className='input' onChange={async e=>{const uid=Number(e.target.value); if(uid){await addGroupMember(g.id,{user_id:uid}); setMembers(p=>({...p,[g.id]:await listGroupMembers(g.id)}))}}}><option value=''>Add user…</option>{users.map(u=><option key={u.id} value={u.id}>{u.username}</option>)}</select></div>:null}
