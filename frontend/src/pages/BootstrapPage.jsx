...
export default function BootstrapPage({ onComplete, setMessage }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Administrator created. Welcome to Proxmox Lab Platform.' })
      await onComplete()
    } catch (error) {
  ...
  return <div className='login-wrap'><form className='login-card' onSubmit={submit}>
    ...
    <p className='muted'>Create the first platform administrator. Enter the one-time token printed by the installer.</p>
