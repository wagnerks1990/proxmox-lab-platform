...
export default function TelemetryPage(){
 ...
 useEffect(()=>{getTelemetrySummary().then(setSummary); getTelemetryEvents().then(setEvents); getAnalyticsSummary().then(setAnalytics)},[])
 return <section><h2>Telemetry</h2><div className='group'><StatCard label='Active Sessions' value={summary.in_memory?.active_session_count}/><StatCard label='Reconnect Attempts' value={summary.in_memory?.reconnect_attempts}/><StatCard label='Failures' value={summary.failures}/><StatCard label='Active Sessions (Analytics)' value={analytics.active_sessions}/></div><div className='muted'>Live: {live.status}</div><RecentEventsTable rows={events}/><LiveEventStreamPanel/></section>
}
