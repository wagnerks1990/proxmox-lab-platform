...
export default function VmsPage({ setMessage }) {
  ...
  return <section className='panel'>
    ...
    <table className='vm-table'><thead><tr><th>VM</th><th>Status</th><th>Node</th><th>IP</th><th>Actions</th><th>Connections</th></tr></thead><tbody>
      {vms.map(v => { const missing = v.status==='missing' || v.status==='error'; const canTerminal = !missing && v.ssh_enabled && hasAccessProtocol(v, 'ssh') && !!v.assigned_ip; const canConsole = !missing && v.console_enabled && hasAccessProtocol(v, 'novnc'); const canRdp = !missing && v.rdp_enabled && hasAccessProtocol(v, 'rdp'); const canSpice = !missing && v.spice_enabled && hasAccessProtocol(v, 'spice'); return <tr key={v.id}><td><div className='vm-title'>{v.vm_name}</div><div className='muted'>VMID {v.vmid}</div></td><td><span className={statusClass(v.status)}>{v.status}</span>{v.status==='missing'?<div className='muted'>Missing in Proxmox. This app record exists, but the VM was not found on the configured Proxmox cluster.</div>:null}</td><td>{v.proxmox_node}</td><td>{v.assigned_ip || '-'}</td><td><div className='group'><button disabled={busy[v.id] || missing} onClick={() => act(v, 'start')}>Start</button><button disabled={busy[v.id] || missing} onClick={() => act(v, 'stop')}>Stop</button><button disabled={busy[v.id] || missing} onClick={() => act(v, 'reboot')}>Reboot</button><button className='btn-danger' disabled={busy[v.id]} onClick={() => act(v, 'delete')}>Delete</button><button disabled={busy[v.id]} onClick={() => act(v, 'status')}>Refresh</button>{missing?<button className='btn-danger' disabled={busy[v.id]} onClick={() => act(v, 'remove_app_record')}>Remove app record</button>:null}</div></td><td><div className='group connection-group'>{canTerminal ? <button className='btn-connection' disabled={launching} onClick={() => launch(v, 'web_terminal')}>WEB TERMINAL</button> : null}{canConsole ? <button className='btn-connection' disabled={launching} onClick={() => launch(v, 'console')}>Console</button> : null}{canRdp ? <button className='btn-connection' disabled={launching} onClick={() => launch(v, 'rdp')}>RDP</button> : null}{canSpice ? <button className='btn-connection' disabled={launching} onClick={() => launch(v, 'spice')}>SPICE</button> : null}</div></td></tr> })}
    </tbody></table>
  </section>
}
