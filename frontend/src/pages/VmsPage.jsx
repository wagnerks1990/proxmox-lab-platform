...
export default function VmsPage({ setMessage }) {
  ...
  const act = async (vm, a) => { setBusy(p => ({ ...p, [vm.id]: true })); try { if (a === 'delete') { const preview = (await api.get(`/vms/${vm.id}/delete-preview`)).data; if (!confirm(`Delete ${preview.vm_name} (VMID ${preview.vmid}) from Proxmox and remove its app record after verification?`)) return; await api.delete(`/vms/${vm.id}`, { params: { confirmation: preview.confirmation } }) } else if (a === 'remove_app_record') { if (!confirm('Remove only the Lab Control app record? This will not delete any Proxmox VM.')) return; await api.delete(`/admin/lab-vms/${vm.id}/app-record`) } else if (a === 'status') { await api.get(`/vms/${vm.id}/status`) } else { await api.post(`/vms/${vm.id}/${a}`) } await load(); setMessage({ type: 'success', text: `${a.toUpperCase()} action queued.` }) } catch (e) { setMessage({ type: 'error', text: JSON.stringify(e?.response?.data?.detail || 'Action failed') }) } finally { setBusy(p => ({ ...p, [vm.id]: false })) } }

  const launch = async (vm, protocol) => {
    if (launching) return
    setLaunching(true); setStatus('launching')
    try {
      if (protocol === 'web_terminal') { const r = await api.get(`/vms/${vm.id}/console/terminal-url`); setSessionId(r.data?.session_id || null); if (r.data?.launch_url) window.open(r.data.launch_url, '_blank', 'noopener,noreferrer'); setStatus('connected'); return }
      ...
      if (protocol === 'console') { const r = await api.get(`/vms/${vm.id}/console/novnc`); setSessionId(r.data?.session_id || null); if (r.data?.novnc_url) window.open(r.data.novnc_url, '_blank', 'noopener,noreferrer'); setStatus('connected') }
