...
export default function VmDetailsDrawer({ vm, onClose, user }) {
  ...
  return <div className='drawer-backdrop' onClick={onClose}><aside className='drawer' onClick={e => e.stopPropagation()}>
    ...
    <div className='panel'><div>VMID: {vm.vmid}</div><div>Status: <StatusBadge status={vm.status} /></div><div>Node: {vm.proxmox_node}</div><div>IP: {vm.assigned_ip || '-'}</div><div>Hostname: {vm.hostname || '-'}</div><div>Protocols: {vm.access_protocols || '-'}</div><div>Created: {vm.created_at || '-'}</div><div>Owner: {(user?.role==='Admin'||user?.role==='Teacher') ? (vm.owner_id||'-') : 'N/A'}</div></div>
  </aside></div>
}
