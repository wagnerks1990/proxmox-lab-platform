...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description='Ensure CONFIG_ENCRYPTION_KEY exists in backend .env')
    parser.add_argument('--env-file', default=str(backend_dir / '.env'))
    parser.add_argument('--create', action='store_true', help='Create env file when missing')
    args = parser.parse_args()

    env_path = Path(args.env_file)
    if not env_path.exists():
        if not args.create:
            print(f'ERROR: {env_path} does not exist. Re-run with --create to create it.')
            return 1
    ...
    if env_values.get('APP_SECRET_KEY', '').strip():
        print('APP_SECRET_KEY exists; generating dedicated CONFIG_ENCRYPTION_KEY for Proxmox token encryption.')
