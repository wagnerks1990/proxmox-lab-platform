...
async def test_inventory_partial_errors(monkeypatch):
    ...
    async def ct(self, n, s="local"):
        return [
            {"node": n[0], "storage_id": "local", "content_type": "vztmpl", "items": []}
        ]
        ...
        return [
            {
                "node": n[0],
                "templates": [
                    {"vmid": 303, "name": "Ubuntu", "template": 1, "status": "stopped"}
                ],
            }
        ]
    ...
    monkeypatch.setattr(
        proxmox_assets.ProxmoxAssetsService, "discover_isos_by_node", iso
    )
    monkeypatch.setattr(
        proxmox_assets.ProxmoxAssetsService, "discover_ct_templates_by_node", ct
    )
    monkeypatch.setattr(
        proxmox_assets.ProxmoxAssetsService, "discover_vm_templates_by_node", vm
    )
