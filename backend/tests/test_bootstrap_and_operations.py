...
from app.services.operation_service import allocate_vmid, claim_operation, enqueue_operation, safe_vm_name
...
def test_first_run_bootstrap_is_one_time_and_sets_http_only_cookie(monkeypatch):
    ...
    try:
        ...
        assert client.post('/api/bootstrap/admin', json={
            'token': 'one-time-bootstrap-secret',
...
def test_vmid_allocator_and_expired_lease_recovery():
    ...
    try:
        ...
        assert safe_vm_name('Student Name', 'Routing / VLAN Lab', 200001) == 'student-name-routing-vlan-lab-200001'
    finally:
        db.close()
        Base.metadata.drop_all(engine)
