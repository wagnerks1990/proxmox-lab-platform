...
async def test_create_rejects_missing_name(monkeypatch):
    ...
    with pytest.raises(Exception) as exc:
        await pools.create_pool(
            payload=payload,
            _user=Obj(id=1),
            organization=OrganizationContext(1, "default", "instructor"),
            db=db,
        )
...
async def test_create_rejects_invalid_template(monkeypatch):
    ...
    with pytest.raises(Exception) as exc:
        await pools.create_pool(
            payload=payload,
            _user=Obj(id=1),
            organization=OrganizationContext(1, "default", "owner"),
            db=db,
        )
    assert getattr(exc.value, "status_code", None) == 422
...
async def test_list_includes_readiness_hints(monkeypatch):
    ...
    async def fake_isos(_):
        return {
            "items": [{"node": "pve-lab-01", "content_id": "local:iso/a.iso"}],
            "warnings": [],
        }

    monkeypatch.setattr(
        pools.ProxmoxBootstrapService,
        "discover_templates",
        lambda self, active: fake_templates(active),
    )
    monkeypatch.setattr(
        pools.ProxmoxBootstrapService,
        "discover_isos",
        lambda self, active: fake_isos(active),
    )
    ...
    db = FakeDB(
        {
            ...
            ProxmoxNode: [
                Obj(node_name="pve-lab-01", status="online"),
                Obj(node_name="pve-lab-02", status="online"),
            ],
            ProxmoxClusterDefault: [Obj(placement_policy="balanced")],
            VMTemplate: [Obj(id=1, source_vmid=303, proxmox_node="pve-lab-01")],
        }
    )
    out = await pools.list_pools(
        _user=Obj(id=1), organization=OrganizationContext(1, "default", "owner"), db=db
    )
    row = out.data[0]
