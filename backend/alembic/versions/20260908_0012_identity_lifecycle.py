...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_auth_sessions_token_id", "auth_sessions", ["token_id"], unique=True
    )
    ...
    op.create_index(
        "ix_auth_login_attempts_blocked_until", "auth_login_attempts", ["blocked_until"]
    )
    ...
    op.add_column(
        "audit_logs", sa.Column("message", sa.String(length=512), nullable=True)
    )
    op.add_column(
        "audit_logs", sa.Column("request_id", sa.String(length=100), nullable=True)
    )
    op.add_column(
        "audit_logs", sa.Column("source_ip", sa.String(length=64), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "audit_logs", "actor_id", existing_type=sa.Integer(), nullable=False
    )

    op.drop_index(
        "ix_auth_login_attempts_blocked_until", table_name="auth_login_attempts"
    )
