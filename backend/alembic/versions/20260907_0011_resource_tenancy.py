...
def upgrade() -> None:
    for table in TENANT_TABLES:
        ...
        op.execute(
            f"UPDATE {table} SET organization_id = (SELECT id FROM organizations WHERE slug = 'default')"
        )
        op.alter_column(
            table, "organization_id", existing_type=sa.Integer(), nullable=False
        )
    ...
    op.add_column(
        "audit_logs", sa.Column("organization_id", sa.Integer(), nullable=True)
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "uq_desktop_pools_organization_name", "desktop_pools", type_="unique"
    )
    op.drop_index("ix_desktop_pools_name", table_name="desktop_pools")
    op.create_index("ix_desktop_pools_name", "desktop_pools", ["name"], unique=True)
    op.drop_constraint(
        "uq_vm_templates_organization_name", "vm_templates", type_="unique"
    )
