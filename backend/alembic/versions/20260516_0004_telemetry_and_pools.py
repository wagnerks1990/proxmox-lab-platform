...
def _ensure_index(table: str, name: str, col: str):
...
def upgrade() -> None:
    insp = inspect(op.get_bind())
    if 'telemetry_events' not in insp.get_table_names():
        op.create_table(
            'telemetry_events',
    ...
    for col in ['event_type','severity','created_at','user_id','vm_id','session_id','request_id']:
        _ensure_index('telemetry_events', f'ix_telemetry_events_{col}', col)
    ...
    for col in ['name','pool_type','enabled','maintenance_mode']:
        _ensure_index('desktop_pools', f'ix_desktop_pools_{col}', col)
