...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "auth_mode", sa.String(length=32), nullable=False, server_default="token"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["cluster_id"],
            ["proxmox_clusters.id"],
            name="fk_proxmox_nodes_cluster_id_clusters",
        ),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["cluster_id"],
            ["proxmox_clusters.id"],
            name="fk_proxmox_cluster_defaults_cluster_id_clusters",
        ),
        sa.UniqueConstraint(
            "cluster_id", name="uq_proxmox_cluster_defaults_cluster_id"
        ),
    )
    op.create_index(
        "ix_proxmox_cluster_defaults_cluster_id",
        "proxmox_cluster_defaults",
        ["cluster_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_proxmox_cluster_defaults_cluster_id", table_name="proxmox_cluster_defaults"
    )
