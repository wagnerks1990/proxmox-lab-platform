...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "max_vms_per_student", sa.Integer(), nullable=False, server_default="1"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=32), nullable=False, server_default="assigned"
        ),
    ...
    op.create_index(
        "ix_lab_assignments_organization_id", "lab_assignments", ["organization_id"]
    )
    ...
    op.create_index(
        "ix_lab_assignments_template_id", "lab_assignments", ["template_id"]
    )
    op.create_index("ix_lab_assignments_status", "lab_assignments", ["status"])
    op.create_index(
        "ix_lab_assignments_student_vm_id",
        "lab_assignments",
        ["student_vm_id"],
        unique=True,
    )
