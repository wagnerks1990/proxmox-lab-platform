...
class ReconciliationPlanningService:
    ...
    def summary(self, organization_id: int | None = None) -> dict:
        ...
        if organization_id is not None:
            pool_query = pool_query.filter(DesktopPool.organization_id == organization_id)
            session_query = session_query.filter(VMSession.organization_id == organization_id)
        pools = pool_query.count()
        stale = session_query.filter(VMSession.state == SessionState.EXPIRED.value).count()
        warnings = 0
