...
class AssetSyncService:
    ...
    def _new_job(
        self, method: str, target_node: str, asset_id: int | None = None, **meta
    ):
        job = AssetSyncJob(
            asset_id=asset_id,
    ...
    def _log(self, job_id: int, level: str, message: str, meta: dict | None = None):
        self.db.add(
            AssetSyncJobEvent(
                ...
                level=level,
                message=message,
                metadata_json=json.dumps(meta or {}, sort_keys=True),
            )
        )
    ...
    async def _poll_task_until_done(self, job: AssetSyncJob, node: str, upid: str):
        ...
        timeout_seconds = max(
            poll_interval, int(settings.asset_sync_download_timeout_seconds or 7200)
        )
        attempt = 0
        ...
        return (
            False,
            f"Timed out waiting for Proxmox download task to complete (elapsed={elapsed_total}s timeout={timeout_seconds}s latest_status={latest_task_status}).",
        )

    async def _verify_on_node(
        self, kind: str, filename: str, node: str, storage_id: str
    ):
        if kind == "iso":
            current = await self.assets.discover_isos_by_node([node], storage_id)
        else:
            current = await self.assets.discover_ct_templates_by_node(
                [node], storage_id
            )
        return any(
            (x.get("filename") == filename)
            for x in (current[0].get("items") if current else [])
        )
    ...
    async def enqueue_iso(
        ...
        for node in target_nodes:
            ...
            self._log(
                ...
                "info",
                "Job queued",
                {"filename": filename, "storage_id": storage_id},
            )
            jobs.append(job)
        self.db.commit()
        return jobs
    ...
    async def process_job(job_id: int):
        ...
        try:
            ...
            if job.method == "download-url-iso":
                ...
                if have:
                    ...
                    svc._log(
                        job.id,
                        "info",
                        f"ISO already present on {node}; verified idempotently.",
                    )
                    db.commit()
                    return
                upid = job.proxmox_upid or await svc.assets.download_url(
                    node, storage_id, "iso", filename, source_url
                )
            else:
                current = await svc.assets.discover_ct_templates_by_node(
                    [node], storage_id
                )
                have = any(
                    (x.get("filename") == filename)
                ...
                if have:
                    ...
                    svc._log(
                        job.id,
                        "info",
                        f"CT template already present on {node}; verified idempotently.",
                    )
                    db.commit()
                    return
                upid = job.proxmox_upid or await svc.assets.download_url(
                    node, storage_id, "vztmpl", filename, source_url
                )

            job.proxmox_upid = str(upid)
            job.state = "syncing"
            svc._log(
                job.id, "info", "download-url submitted", {"upid": job.proxmox_upid}
            )
            db.commit()
            ...
            if not ok:
                ...
                db.commit()
                return
            have_after = await svc._verify_on_node(
                "iso" if job.method == "download-url-iso" else "ct_template",
                filename,
                node,
                storage_id,
            )
    ...
    async def sync_vm_template(
        ...
        for node in target_nodes:
            ...
            self._finish(
                job,
                "failed",
                "VM template sync requires configured command runner (qm/pvesh) and is not enabled in this environment.",
            )
            self._log(
                job.id,
                "error",
                job.error,
                {"template_name": template_name, "storage_id": storage_id},
            )
            jobs.append(job)
        self.db.commit()
        return jobs
