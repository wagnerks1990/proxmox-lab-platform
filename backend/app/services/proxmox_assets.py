...
class ProxmoxAssetsService:
    ...
    def client(self):
        if self._client is None:
            self._client = ProxmoxClient(self.cluster_id)
        return self._client

    async def _get(self, path: str, params: dict | None = None):
        async with httpx.AsyncClient(
            verify=self.client.verify_ssl, timeout=60, headers=self.client.headers
        ) as http:
    ...
    async def _post(self, path: str, data: dict):
        async with httpx.AsyncClient(
            verify=self.client.verify_ssl, timeout=120, headers=self.client.headers
        ) as http:
    ...
    async def discover_isos_by_node(self, nodes: list[str], storage_id: str = "local"):
        ...
        for node in nodes:
            rows = await self._get(
                f"/nodes/{node}/storage/{storage_id}/content", {"content": "iso"}
            )
            out.append(
                {
                    "node": node,
                    ...
                    "items": [
                        {
                            "volid": r.get("volid"),
                            "filename": (r.get("volid") or "").split("/")[-1]
                            if r.get("volid")
                            else None,
    ...
    async def discover_ct_templates_by_node(
        self, nodes: list[str], storage_id: str = "local"
    ):
        out = []
        for node in nodes:
            rows = await self._get(
                f"/nodes/{node}/storage/{storage_id}/content", {"content": "vztmpl"}
            )
            out.append(
                {
                    "node": node,
    ...
    async def download_url(
        ...
        return await self._post(
            f"/nodes/{target_node}/storage/{storage_id}/download-url",
            {
                "content": content,
                "filename": filename,
                "url": source_url,
