...
async def health_summary(db: Session):
    ...
    defaults = db.query(ProxmoxClusterDefault).filter(ProxmoxClusterDefault.cluster_id == active_cluster.id).first() if active_cluster else None
    online_count = db.query(ProxmoxNode).filter(ProxmoxNode.cluster_id == active_cluster.id, ProxmoxNode.status.in_(['online','up'])).count() if active_cluster else 0
    try:
