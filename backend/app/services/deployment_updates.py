...
class DeploymentUpdateService:
    ...
    def get_settings(self) -> DeploymentUpdateSettings:
        row = (
            self.db.query(DeploymentUpdateSettings)
            .order_by(DeploymentUpdateSettings.id.asc())
            .first()
        )
        if row is None:
    ...
    def update_settings(self, payload: dict, actor_id: int) -> DeploymentUpdateSettings:
        ...
        if "automatic_updates" in payload:
            ...
            if requested and (
                not settings.updater_allow_automatic
                or not settings.updater_require_signed_commits
            ):
                raise HTTPException(
                    status_code=409,
                    detail="Automatic updates require UPDATER_ALLOW_AUTOMATIC=true and UPDATER_REQUIRE_SIGNED_COMMITS=true in host policy.",
                )
        ...
        if "check_interval_minutes" in payload:
            ...
            if not 15 <= interval <= 10080:
                raise HTTPException(
                    status_code=422,
                    detail="Check interval must be between 15 and 10080 minutes",
                )
        ...
        if "maintenance_hour_utc" in payload:
            ...
            if not 0 <= hour <= 23:
                raise HTTPException(
                    status_code=422, detail="Maintenance hour must be 0 through 23 UTC"
                )
        ...
        self.db.add(
            AuditLog(
                actor_id=actor_id,
                action="deployment.update_settings",
    ...
    def agent_request(
        ...
        try:
            ...
            with httpx.Client(
                transport=transport, base_url="http://updater", timeout=30
            ) as client:
                response = client.request(
                    method,
                    path,
                    json=payload,
                    headers={"X-Updater-Token": settings.updater_token},
                )
    ...
    def run(
        self, action: str, actor_id: int | None, target_ref: str | None = None
    ) -> dict:
        ...
        if target_ref:
            if action == "apply" and not re.fullmatch(r"[0-9a-f]{40}", target_ref):
                raise HTTPException(
                    status_code=422,
                    detail="Apply requires the exact 40-character commit SHA returned by update check",
                )
            if action != "apply" and (
                not _REF_RE.fullmatch(target_ref) or ".." in target_ref
            ):
                raise HTTPException(
                    status_code=422, detail="Invalid target Git reference"
                )
            payload["target_ref"] = target_ref
        elif action == "apply":
            raise HTTPException(
                status_code=422,
                detail="Run update check and provide its exact target commit SHA",
            )
        run = DeploymentUpdateRun(
            requested_by=actor_id, action=action, status="running"
        )
        self.db.add(run)
        self.db.commit()
        self.db.refresh(run)
    ...
    def status(self) -> dict:
        ...
        latest = (
            self.db.query(DeploymentUpdateRun)
            .order_by(DeploymentUpdateRun.id.desc())
            .first()
        )
        ...
        if (
            ...
            and latest.agent_operation_id
            and operation.get("id") == latest.agent_operation_id
            and operation.get("status") in {"succeeded", "failed"}
        ):
            latest.status = (
                "success" if operation["status"] == "succeeded" else "failed"
            )
            latest.finished_at = datetime.utcnow()
            operation_result = operation.get("result") or {}
            latest.from_version = (
                operation_result.get("from_version") or latest.from_version
            )
            latest.to_version = operation_result.get("to_version") or latest.to_version
            latest.backup_path = (
                operation_result.get("backup_path") or latest.backup_path
            )
            latest.details = str(
