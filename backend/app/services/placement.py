...
def _score(node: dict[str, Any], running_vm_count: int) -> tuple[float, float, int, str]:
...
def choose_cluster_node(
    ...
    policy = (placement_policy or '').strip() or ('prefer_default_then_balance' if default_node else 'balanced')
    online_nodes = [n for n in nodes if _is_online(n)]
    if allowed_nodes is not None:
        online_nodes = [n for n in online_nodes if (n.get('node_name') in allowed_nodes)]
    ...
    if policy == 'manual':
        if not default_node:
            raise PlacementError('Placement policy manual requires a configured default node.')
        if default_node not in online_names:
            raise PlacementError(f'Placement policy manual selected {default_node}, but that node is offline or unavailable.')
        return PlacementDecision(default_node, 'manual default node', {'policy': policy})

    if policy == 'prefer_default_then_balance' and default_node and default_node in online_names:
        return PlacementDecision(default_node, 'preferred default node is online', {'policy': policy})

    scored = sorted(
        online_nodes,
        key=lambda n: _score(n, running_counts_by_node.get(n.get('node_name') or '', 0)),
    )
    selected = scored[0]
