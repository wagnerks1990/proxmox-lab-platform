...
class ProxmoxResourceStatsService:
    ...
    async def _collect_from_client(
        self, client: ProxmoxClient, cluster: ProxmoxCluster | None
    ) -> dict[str, Any]:
        warnings: list[str] = []
        ...
        for n in node_resources:
            ...
            cpu_ratio = n.get("cpu")
            cpu_usage_percent = (
                round(float(cpu_ratio) * 100.0, 2) if cpu_ratio is not None else None
            )
            node_map[name] = {
                "name": name,
        ...
        online_nodes = len(
            [n for n in nodes if (n.get("status") or "").lower() in {"online", "up"}]
        )
        cpu_weighted = None
        if total_cpu_cores > 0:
            weighted_sum = sum(
                ((n.get("cpu_usage_percent") or 0.0) * (n.get("cpu_count") or 0))
                for n in nodes
            )
