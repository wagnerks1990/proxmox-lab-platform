...
from app.services.security import create_access_token, hash_password, new_token_id, validate_password, verify_password
...
def _rate_limit_row(db: Session, username: str, client_ip: str | None) -> AuthLoginAttempt | None:
    return db.query(AuthLoginAttempt).filter(AuthLoginAttempt.key_hash == _attempt_key(username, client_ip)).with_for_update().first()
...
def _record_login_failure(db: Session, username: str, client_ip: str | None) -> None:
    ...
    if not row:
        row = AuthLoginAttempt(key_hash=_attempt_key(username, client_ip), failures=0, first_failure_at=now)
        db.add(row)
...
def issue_session(db: Session, user: User, *, client_ip: str | None = None, user_agent: str | None = None) -> str:
    now = _utcnow()
    ...
    return create_access_token(user.username, token_id=token_id, token_version=user.token_version or 1)
...
def login_user(
    ...
    *,
    ...
    credentials_valid = verify_password(password, user.password_hash if user else DUMMY_PASSWORD_HASH)
    if not user or not credentials_valid:
        ...
        record_audit_event(
            ...
            source_ip=client_ip,
            metadata={'username_hash': hashlib.sha256(username.strip().lower().encode()).hexdigest()},
        )
        db.commit()
        raise HTTPException(status_code=401, detail='Invalid credentials')
    if not getattr(user, 'is_active', True):
        record_audit_event(db, actor_id=user.id, action='identity.login', target_type='user', target_id=str(user.id), outcome='denied', message='Account disabled', request_id=request_id, source_ip=client_ip)
        db.commit()
    ...
    record_audit_event(db, actor_id=user.id, action='identity.login', target_type='auth_session', target_id='new', request_id=request_id, source_ip=client_ip)
    db.commit()
    return token
...
def revoke_all_sessions(db: Session, user: User, *, reason: str) -> int:
    now = _utcnow()
    rows = db.query(AuthSession).filter(AuthSession.user_id == user.id, AuthSession.revoked_at.is_(None)).all()
    for row in rows:
        row.revoked_at = now
...
def change_password(db: Session, user: User, current_password: str, new_password: str) -> str:
    ...
    record_audit_event(db, actor_id=user.id, action='identity.password_changed', target_type='user', target_id=str(user.id))
    db.commit()
    return token
