...
def enforce_organization_role(
    context: OrganizationContext, minimum_role: str
) -> OrganizationContext:
    if not organization_role_at_least(context, minimum_role):
        raise HTTPException(
            status_code=403,
            detail=f"Organization role {minimum_role} or higher is required",
        )
    return context
...
def resolve_organization_context(
    ...
    if requested_organization_id is not None:
        ...
        if role == ROLE_ADMIN:
            return OrganizationContext(
                organization.id, organization.slug, "owner", True
            )
    ...
    if len(valid) == 1:
        ...
        return OrganizationContext(
            organization.id,
            organization.slug,
            normalize_organization_role(membership.role),
        )
    if len(valid) > 1:
        raise HTTPException(
            status_code=400,
            detail="X-Organization-ID is required for users with multiple organizations",
        )
    if role == ROLE_ADMIN:
        raise HTTPException(
            status_code=400,
            detail="X-Organization-ID is required for platform administrators",
        )
