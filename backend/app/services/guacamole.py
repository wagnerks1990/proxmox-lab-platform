...
async def _auth(client: httpx.AsyncClient) -> GuacamoleAuth:
    r = await client.post(
        ...
        data={
            ...
            "password": settings.guacamole_admin_password,
        },
    )
    ...
    if not token or not datasource:
        raise GuacamoleError(
            "Guacamole authentication response missing authToken or dataSource"
        )
...
def _encode_client_id(connection_id: str, datasource: str) -> str:
    return base64.b64encode(
        f"{connection_id}\x00c\x00{datasource}".encode("utf-8")
    ).decode("utf-8")
...
async def create_or_update_connection(vm, protocol: str) -> tuple[str, str]:
    ...
    async with httpx.AsyncClient(
        timeout=10.0, verify=settings.guacamole_verify_ssl
    ) as client:
        ...
        if existing.status_code >= 400:
            raise GuacamoleError(
                f"Failed listing connections: HTTP {existing.status_code}"
            )
        ...
        payload = {
            ...
            "parameters": {
                "hostname": hostname,
                "port": str(port),
                "username": vm.default_username or "",
            },
            "attributes": {},
        }
        ...
        if target_id:
            upd = await client.put(
                f"{endpoint}/{target_id}", params=params, json=payload
            )
            if upd.status_code >= 400:
                raise GuacamoleError(
                    f"Failed updating connection: HTTP {upd.status_code}"
                )
        else:
            create = await client.post(endpoint, params=params, json=payload)
            if create.status_code >= 400:
                raise GuacamoleError(
                    f"Failed creating connection: HTTP {create.status_code}"
                )
            target_id = str(create.json().get("identifier") or create.json().get("id"))
            if not target_id:
                raise GuacamoleError(
                    "Guacamole create connection response missing identifier"
                )
