...
from app.architecture.events import (
    bus,
    DomainEvent,
    SESSION_CREATED,
    SESSION_EXPIRED,
...
from app.architecture.state_machines import (
    SessionState,
    SESSION_TRANSITIONS,
    validate_transition,
)
...
class SessionService:
    ...
    def _set_state(
        self, row: VMSession, next_state: SessionState, *, reason: str | None = None
    ) -> bool:
    ...
    def create_launching_session(
        self,
        user: User,
        vm: StudentVM,
        protocol: str,
        connection_launch_id: int | None = None,
        ...
        row = VMSession(
            ...
            vm_id=vm.id,
            protocol=protocol,
            state=SessionState.LAUNCHING.value,
            node=vm.proxmox_node,
            proxmox_vmid=vm.vmid,
            connection_launch_id=connection_launch_id,
    ...
    def get_session_for_user(
        ...
        if organization_role == "student" or (
            organization_role is None and role == ROLE_STUDENT
        ):
            q = q.filter(VMSession.user_id == user.id)
        elif (
            organization_role not in {"instructor", "admin", "owner"}
            and role not in STAFF_ROLES
        ):
            return None
        return q.first()
    ...
    def heartbeat(
        self, session_id: int, next_state: SessionState | None = None
    ) -> VMSession | None:
        row = self.db.query(VMSession).filter(VMSession.id == session_id).first()
        if not row:
            return None
    ...
    def get_active_sessions(self, user_id: int | None = None):
        q = self.db.query(VMSession).filter(
            VMSession.state == SessionState.ACTIVE.value
        )
    ...
    def expire_stale_sessions(self, stale_seconds: int = 900) -> int:
        ...
        rows = (
            ...
            .filter(
                ...
                VMSession.last_heartbeat_at.isnot(None),
                VMSession.last_heartbeat_at < cutoff,
            )
            .all()
        )
        count = 0
        ...
        if count:
            safe_commit(self.db)
            bus.publish(
                DomainEvent(name=STALE_CLEANUP, payload={"expired_count": count})
            )
        return count

    def reconnect(
        self, session_id: int, reconnect_token: str, user_id: int
    ) -> VMSession | None:
        row = self.db.query(VMSession).filter(VMSession.id == session_id).first()
        if not row or not self.verify_reconnect(
            reconnect_token, session_id, user_id, row.protocol
        ):
            return None
    ...
    def analytics_counts(self) -> dict[str, int]:
        ...
        failed = (
            self.db.query(func.count(VMSession.id))
            .filter(VMSession.state == SessionState.FAILED.value)
            .scalar()
            or 0
        )
    ...
    def create_launch(
        ...
        session_state: SessionState = SessionState.LAUNCHING,
    ) -> ConnectionLaunch:
        row = ConnectionLaunch(
            actor_id=user.id,
            vm_id=vm.id,
            protocol=protocol,
            status=status,
            details=details or f"state={session_state.value}",
        )
        self.db.add(row)
        self.db.flush()
        return row
    ...
    def issue_reconnect_token(
        self, session_id: int, user_id: int, protocol: str
    ) -> str:
        secret = settings.reconnect_token_secret or settings.jwt_secret_key
        return create_reconnect_token(
            secret=secret,
            session_id=session_id,
            user_id=user_id,
            protocol=protocol,
            ttl_seconds=settings.reconnect_token_ttl_seconds,
        )

    def verify_reconnect(
        self, token: str, session_id: int, user_id: int, protocol: str
    ) -> bool:
        secret = settings.reconnect_token_secret or settings.jwt_secret_key
        try:
            payload = verify_reconnect_token(
                token=token,
                secret=secret,
                session_id=session_id,
                user_id=user_id,
    ...
    def expire_reconnecting_sessions(self, timeout_seconds: int) -> int:
        ...
        rows = (
            self.db.query(VMSession)
            .filter(
                VMSession.state == SessionState.RECONNECTING.value,
                VMSession.updated_at < cutoff,
            )
            .all()
        )
        count = 0
    ...
    def fail_stuck_launching(self, timeout_seconds: int) -> int:
        ...
        rows = (
            self.db.query(VMSession)
            .filter(
                VMSession.state == SessionState.LAUNCHING.value,
                VMSession.created_at < cutoff,
            )
            .all()
        )
        count = 0
