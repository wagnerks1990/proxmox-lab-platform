...
class ValidationService:
    ...
    def run_checks(self) -> list[dict]:
        ...
        checks.append(
            self._ok(
                "workers",
                "scheduler_enabled",
                f"scheduler enabled={settings.worker_scheduler_enabled}",
            )
        )
        checks.append(
            self._ok(
                "telemetry",
                "event_stream",
                "event stream configured (in-process memory mode)",
            )
        )
        checks.append(
            self._ok(
                ...
                f"replay backend={settings.replay_store_backend}",
            )
        )
        checks.append(
            self._ok(
                "security",
                "worker_lock_backend",
                f"worker lock backend={settings.worker_lock_backend}",
            )
        )
        checks.append(
            self._ok(
                ...
                f"maintenance pools={self.db.query(func.count(DesktopPool.id)).filter(DesktopPool.maintenance_mode.is_(True)).scalar() or 0}",
            )
        )
        ...
        checks.append(
            self._fail(
                ...
                "reconnect_secret",
                "warning",
                "reconnect token secret falls back to jwt secret",
                "Set RECONNECT_TOKEN_SECRET explicitly",
            )
            if weak
            else self._ok(
                "security", "reconnect_secret", "reconnect token secret configured"
            )
        )
        return checks
    ...
    def _metric_check(self, name, value, cat):
        ...
        return {
            ...
            "status": status,
            "severity": sev,
            "message": f"{name}={int(value)}",
            "suggested_fix": "Investigate if non-zero" if value else "None",
