...
from app.models.models import AuditLog, DurableOperation, LabAssignment, StudentVM, VMTemplate, VmidAllocator
...
def _is_not_found(exc: Exception) -> bool:
    ...
    return any(marker in message for marker in ('404', 'not found', 'does not exist'))
...
def allocate_vmid(db: Session, scope: str = 'proxmox-cluster') -> int:
    allocator = db.query(VmidAllocator).filter(VmidAllocator.scope == scope).with_for_update().first()
    if not allocator:
        allocator = VmidAllocator(scope=scope, next_value=200000)
        db.add(allocator)
        db.flush()
    largest = db.query(StudentVM.vmid).order_by(StudentVM.vmid.desc()).limit(1).scalar() or 199999
...
def enqueue_operation(
    ...
    *,
    ...
    existing = db.query(DurableOperation).filter(DurableOperation.idempotency_key == key).first()
    if existing:
        return existing
...
def claim_operation(db: Session, worker_id: str) -> DurableOperation | None:
    now = utcnow()
    row = db.query(DurableOperation).filter(
        or_(
            DurableOperation.state == 'queued',
            (DurableOperation.state == 'running') & (DurableOperation.lease_expires_at < now),
        ),
        or_(DurableOperation.run_after.is_(None), DurableOperation.run_after <= now),
    ).order_by(DurableOperation.created_at, DurableOperation.id).with_for_update(skip_locked=True).first()
    if not row:
        return None
    row.state = 'running'
...
async def execute_operation(db: Session, row: DurableOperation) -> None:
    ...
    if row.operation_type == 'vm.create':
        ...
        try:
            ...
            if not row.proxmox_upid:
                raise RuntimeError(f'VMID {vm.vmid} already exists in Proxmox and is not owned by this operation')
        except Exception as exc:
            if not _is_not_found(exc):
                raise
            response = await proxmox.clone_vm(vm.proxmox_node, template.source_vmid, vm.vmid, vm.vm_name)
            row.proxmox_upid = str(response.get('data') or '') or None
            db.commit()
            if row.proxmox_upid:
                task = await proxmox.wait_for_task(vm.proxmox_node, row.proxmox_upid)
                if task.get('exitstatus') not in ('OK', None):
                    raise RuntimeError(f'Clone task failed: {task.get("exitstatus") or "unknown"}')
            live = await proxmox.get_vm_status(vm.proxmox_node, vm.vmid)
        ...
        if payload.get('auto_start') and vm.status != 'running':
            ...
            if start_upid:
                await proxmox.wait_for_task(vm.proxmox_node, start_upid)
            vm.status = (await proxmox.get_vm_status(vm.proxmox_node, vm.vmid)).get('status', 'running')
        assignment = db.query(LabAssignment).filter(LabAssignment.student_vm_id == vm.id).first()
        if assignment:
            assignment.status = 'ready'
        result = {'vm_id': vm.id, 'vmid': vm.vmid, 'status': vm.status, 'node': vm.proxmox_node}
        ...
        row.proxmox_upid = (str(response.get('data') or '') or None) if isinstance(response, dict) else None
        if row.proxmox_upid:
            await proxmox.wait_for_task(vm.proxmox_node, row.proxmox_upid)
        vm.status = (await proxmox.get_vm_status(vm.proxmox_node, vm.vmid)).get('status', vm.status)
        ...
        if not vm:
            ...
            try:
                ...
                if live.get('status') == 'running':
                    ...
                    if stop_upid:
                        await proxmox.wait_for_task(vm.proxmox_node, stop_upid)
            ...
            if assignment:
                ...
                assignment.status = 'expired' if payload.get('expiration_cleanup') else 'assigned'
    ...
    db.add(AuditLog(organization_id=row.organization_id, actor_id=row.requested_by, action=row.operation_type, target_type=row.target_type, target_id=row.target_id or '', outcome='success'))
    db.commit()
    if row.operation_type == 'vm.start':
        bus.publish(DomainEvent(name=VM_STARTED, payload={'organization_id': row.organization_id, 'vm_id': int(row.target_id), 'actor_id': row.requested_by}))
...
def fail_or_retry(db: Session, row: DurableOperation, exc: Exception) -> None:
    ...
    if row.attempts < settings.operation_max_attempts:
        ...
        row.state = 'failed'
        row.finished_at = utcnow()
        vm = db.query(StudentVM).filter(StudentVM.id == int(row.target_id)).first() if row.target_type == 'student_vm' and row.target_id else None
        if vm and row.operation_type == 'vm.create':
            vm.status = 'error'
    db.add(AuditLog(organization_id=row.organization_id, actor_id=row.requested_by, action=row.operation_type, target_type=row.target_type, target_id=row.target_id or '', outcome='failure', message=row.error))
    db.commit()
