...
def list_operations(limit: int = 50, user=Depends(require_role('Student', 'Teacher', 'Admin')), db: Session = Depends(get_db), organization: OrganizationContext = Depends(get_current_organization)):
    q = db.query(DurableOperation).filter(DurableOperation.organization_id == organization.id)
...
def get_operation(operation_id: int, user=Depends(require_role('Student', 'Teacher', 'Admin')), db: Session = Depends(get_db), organization: OrganizationContext = Depends(get_current_organization)):
    row = db.query(DurableOperation).filter(DurableOperation.id == operation_id, DurableOperation.organization_id == organization.id).first()
    if not row or (not organization_role_at_least(organization, 'instructor') and row.requested_by != user.id):
