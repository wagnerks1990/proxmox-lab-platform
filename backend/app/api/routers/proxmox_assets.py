...
from app.models.models import ProxmoxCluster, AssetSyncJob, AssetSyncJobEvent
...
async def assets_inventory(_user=Depends(require_role('Admin')), db: Session = Depends(get_db)):
    ...
    iso = await svc.discover_isos_by_node(node_names, 'local')
    ct = await svc.discover_ct_templates_by_node(node_names, 'local')
    vm = await svc.discover_vm_templates_by_node(node_names)
    return {'nodes': nodes, 'storages': ['local', 'local-lvm'], 'iso_by_node': iso, 'ct_templates_by_node': ct, 'vm_templates_by_node': vm}
...
async def assets_readiness(_user=Depends(require_role('Admin')), db: Session = Depends(get_db)):
    ...
    return {
        ...
        ] if constrained else ['All required assets are present on all online nodes.']
    }
...
async def sync_job(job_id: int, _user=Depends(require_role('Admin')), db: Session = Depends(get_db)):
    ...
    return {
        'job': {
            'id': job.id, 'state': job.state, 'method': job.method, 'target_node': job.target_node,
            'source_node': job.source_node, 'source_vmid': job.source_vmid, 'target_vmid': job.target_vmid,
            'proxmox_upid': job.proxmox_upid, 'error': job.error,
        },
