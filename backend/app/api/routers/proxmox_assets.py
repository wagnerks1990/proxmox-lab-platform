...
async def asset_source_url(
    ...
    base = (
        ...
        if resolved_kind == "iso"
        else settings.asset_source_ct_base_url
        if resolved_kind in {"ct", "ct-template", "vztmpl", "ct_template"}
        else None
    )
    if not base:
        return {
            ...
            "kind": resolved_kind,
            "filename": filename,
            "message": "Asset source base URL is not configured.",
        }
    return {
        "ok": True,
        "kind": resolved_kind,
        "filename": filename,
        "source_url": f"{base.rstrip('/')}/{quote(filename)}",
    }


@router.get("/admin/proxmox/assets/inventory")
async def assets_inventory(
    ...
    return {
        ...
        "iso_by_node": iso,
        "ct_templates_by_node": ct,
        "vm_templates_by_node": vm,
...
async def assets_readiness(
    ...
    constrained = sorted(
        set(
            list(missing_isos.keys())
            + list(missing_ct.keys())
            + list(missing_vm.keys())
        )
    )
    ready = sorted([n for n in nodes if n not in constrained])
    required_assets = (
        db.query(AssetCatalog).filter(AssetCatalog.is_required.is_(True)).all()
    )
    status = (
        "PASS"
        if (len(constrained) == 0 and len(required_assets) > 0)
        else ("WARN" if len(nodes) > 0 else "FAIL")
    )
    ...
    return {
        ...
        "vm_template_vmid_by_node": {
            t.get("name"): {
                g.get("node"): x.get("vmid")
                for g in inv.get("vm_templates_by_node", [])
                for x in g.get("templates", [])
                if x.get("name") == t.get("name")
            }
            for g in inv.get("vm_templates_by_node", [])
        ...
        if constrained
            ...
            ["No required assets are configured in asset_catalog yet."]
            if len(required_assets) == 0
            else ["All required assets are present on all online nodes."]
        ),
    }
...
async def sync_iso(
    payload: SyncIsoRequest,
    _user=Depends(require_role("Admin")),
    db: Session = Depends(get_db),
):
    jobs = await AssetSyncService(db).enqueue_iso(
        payload.filename, payload.storage_id, payload.source_url, payload.target_nodes
    )
    return {
        "jobs": [
            {
...
async def sync_ct_template(
    payload: SyncCtTemplateRequest,
    _user=Depends(require_role("Admin")),
    db: Session = Depends(get_db),
):
    jobs = await AssetSyncService(db).enqueue_ct_template(
        payload.filename, payload.storage_id, payload.source_url, payload.target_nodes
    )
    return {
        "jobs": [
...
async def sync_vm_template(
    payload: SyncVmTemplateRequest,
    _user=Depends(require_role("Admin")),
    db: Session = Depends(get_db),
):
    jobs = await AssetSyncService(db).sync_vm_template(
        payload.source_node,
        payload.source_vmid,
...
async def sync_job(
    job_id: int, _user=Depends(require_role("Admin")), db: Session = Depends(get_db)
):
    job = db.query(AssetSyncJob).filter(AssetSyncJob.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="job not found")
    events = (
        db.query(AssetSyncJobEvent)
        .filter(AssetSyncJobEvent.job_id == job_id)
        .order_by(AssetSyncJobEvent.id.asc())
        .all()
    )
    return {
        "job": {
