...
def session_activity(user=Depends(get_current_user), db: Session = Depends(get_db), organization: OrganizationContext = Depends(require_organization_role('instructor'))):
...
def session_heartbeat(id: int, payload: SessionHeartbeatRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db), organization: OrganizationContext = Depends(get_current_organization)):
    ...
    row = svc.get_session_for_user(id, user, organization.id, organization.role)
    if not row:
