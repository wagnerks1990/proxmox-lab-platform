    ...
    if row.template_vmid:
        tpl = (
            ...
            .filter(
                ...
                VMTemplate.organization_id == row.organization_id,
            )
            .first()
        )
        row.linked_vm_count = (
            db.query(StudentVM)
            .filter(
                StudentVM.template_id == tpl.id,
                StudentVM.organization_id == row.organization_id,
            )
            .count()
            if tpl
            else 0
...
async def _attach_pool_readiness_hints(db: Session, row: DesktopPool) -> DesktopPool:
    ...
    if not active:
        ...
        row.recommended_next_steps = [
            "Configure and activate a Proxmox cluster before using this pool."
        ]
        return row
    ...
    nodes = [
        ...
        if (n.status or "").lower() in {"online", "up"}
    ]
    template_nodes = {
        str(t.get("node"))
        for t in templates
        if row.template_vmid and int(t.get("vmid", -1)) == int(row.template_vmid)
    }
    row.asset_ready_nodes = sorted(list(template_nodes)) if row.template_vmid else []
    row.constrained_nodes = (
        sorted([n for n in nodes if n not in template_nodes])
        if row.template_vmid
        else []
    )
    if row.template_vmid:
        row.missing_templates_by_node = {
            n: [row.template_vmid] for n in row.constrained_nodes
        }
    else:
        row.missing_templates_by_node = {}
    ...
    defaults = (
        ...
        .filter(ProxmoxClusterDefault.cluster_id == active.id)
        .first()
    )
...
async def create_pool(
    ...
    if size_min < 0 or size_max < 0 or size_min > size_max:
        raise HTTPException(
            ...
            detail={
                "errors": [
                    "size_min/size_max must be non-negative and size_min <= size_max"
                ]
            },
        )
    ...
    if data.get("template_vmid"):
        tpl = (
            db.query(VMTemplate)
            .filter(
                VMTemplate.source_vmid == data["template_vmid"],
        ...
        if not tpl:
            raise HTTPException(
                status_code=422,
                detail="Import Proxmox templates before creating pools.",
            )
    ...
    _write_audit(
        ...
        "POOL_CREATED",
        str(row.id),
        f"Pool {row.name} created",
    )
    return ApiEnvelope(
        success=True,
        data=await _attach_pool_readiness_hints(db, _attach_pool_counts(db, row)),
    )
...
async def get_pool(
    ...
    row = (
        db.query(DesktopPool)
        .filter(DesktopPool.id == id, DesktopPool.organization_id == organization.id)
        .first()
    )
    if not row:
...
def delete_pool(
    ...
    if attached > 0 and not force:
        raise HTTPException(
            status_code=409,
            detail="Pool may have VM dependencies; pass force=true to remove app pool record",
        )
    _write_audit(
        db,
        _user.id,
    ...
    return ApiEnvelope(
        ...
        data={
            "deleted": True,
            "id": id,
            "message": "Pool app record deleted. No Proxmox VMs were changed.",
        },
    )


@router.patch("/pools/{id}/enabled", response_model=ApiEnvelope[PoolOut])
def patch_pool_enabled(
    id: int,
    payload: dict,
    _user=Depends(get_current_user),
    db: Session = Depends(get_db),
...
def pool_members(
    ...
    members = (
        []
        if not template
        else db.query(StudentVM)
        .filter(
            StudentVM.organization_id == organization.id,
            StudentVM.template_id == template.id,
        )
        .order_by(StudentVM.id)
        .all()
    )
    return ApiEnvelope(
        success=True,
...
async def pool_readiness(
    ...
    nodes = [
        n.node_name
        for n in db.query(ProxmoxNode)
        .filter(ProxmoxNode.cluster_id == (active.id if active else -1))
        .all()
    ]
            ...
            template_nodes = sorted(
                {
                    str(t.get("node"))
                    for t in discovered
                    if t.get("vmid") == row.template_vmid and t.get("node")
                }
            )
            missing_nodes = [n for n in nodes if n not in template_nodes]
            if not template_nodes:
                failures.append(
                    f"Template VMID {row.template_vmid} was not found in live Proxmox discovery."
                )
            elif missing_nodes:
                warnings.append(
                    f"Template VMID {row.template_vmid} is not on all nodes; missing: {', '.join(missing_nodes)}."
                )
    status = "PASS"
    if failures:
        status = "FAIL"
    ...
    defaults = (
        db.query(ProxmoxClusterDefault)
        .filter(ProxmoxClusterDefault.cluster_id == (active.id if active else -1))
        .first()
    )
    recommended_next_steps = []
    if failures:
        recommended_next_steps.append(
            "Resolve FAIL conditions before provisioning from this pool."
        )
    if any("not on all nodes" in w for w in warnings):
        recommended_next_steps.append(
            "Balanced placement may be limited until template availability is expanded or shared storage is used."
        )
    return ApiEnvelope(
        success=True,
        data={
