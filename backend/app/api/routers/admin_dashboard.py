...
async def dashboard_summary(
        ...
        warnings.append(
            {
                "severity": "warning",
                "title": "No templates",
                "message": "No app templates are imported yet.",
                "link": "/admin/proxmox-inventory",
            }
        )
    if (
        ...
        warnings.append(
            {
                "severity": "warning",
                "title": "No pools",
                "message": "No desktop pools are configured.",
                "link": "/pools",
            }
        )
    if active and not getattr(defaults, "default_node", None):
        warnings.append(
            {
                "severity": "warning",
                "title": "Default node missing",
                "message": "Cluster default_node is not configured.",
                "link": "/admin/proxmox-setup",
            }
        )
    ...
    return {
        "health": {
            ...
            "database": {
                ...
                "error": health.get("database", {}).get("error"),
            },
            "proxmox": {
                ...
                "default_template_vmid": getattr(
                    defaults, "default_template_vmid", None
                )
                if defaults
                else None,
            },
        ...
        "counts": {
            ...
            "vms": len(vms),
            "running_vms": len(
                [v for v in vms if (v.status or "").lower() == "running"]
            ),
            "nodes": len(nodes),
            "online_nodes": len(online_nodes),
            "sessions": db.query(VMSession)
            .filter(VMSession.organization_id == organization.id)
            .count(),
            "events": db.query(AuditLog)
            .filter(AuditLog.organization_id == organization.id)
            .count(),
        },
        "capacity": {
            "cpu_percent": None,
