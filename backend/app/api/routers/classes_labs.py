...
def _class_or_404(
    db: Session, class_id: int, organization: OrganizationContext, user
) -> Class:
...
def _validate_instructor(db: Session, organization_id: int, user_id: int) -> None:
    ...
    if not membership:
        raise HTTPException(
            status_code=422,
            detail="instructor must be an active instructor in this organization",
        )
...
def list_classes(
    ...
    return ApiEnvelope(
        success=True,
        data=_class_query(db, organization, _user).order_by(Class.id.desc()).all(),
    )
...
def create_class(
    ...
    if organization_role_at_least(organization, "admin"):
        ...
        if data.get("instructor_id") not in (None, _user.id):
            raise HTTPException(
                status_code=403,
                detail="Instructors may only create classes assigned to themselves",
            )
...
def patch_class(
    ...
    if "instructor_id" in changes:
        if not organization_role_at_least(organization, "admin"):
            raise HTTPException(
                status_code=403,
                detail="Only organization administrators may reassign a class",
            )
...
def delete_class(
    ...
    row = _class_or_404(db, id, organization, _user)
    if (
        db.query(Enrollment).filter(Enrollment.class_id == id).first()
        or db.query(Lab).filter(Lab.class_id == id).first()
    ):
        raise HTTPException(
            status_code=409, detail="Cannot delete class with labs or enrollments"
        )
    db.delete(row)
    db.commit()
...
@router.get(
    "/admin/classes/{id}/enrollments", response_model=ApiEnvelope[list[EnrollmentOut]]
)
def list_enrollments(
    ...
    return ApiEnvelope(
        success=True, data=db.query(Enrollment).filter(Enrollment.class_id == id).all()
    )


@router.post(
    "/admin/classes/{id}/enrollments", response_model=ApiEnvelope[EnrollmentOut]
)
def add_enrollment(
    id: int,
    ...
    if (
        ...
        raise HTTPException(
            status_code=422, detail="user must be an active member of this organization"
        )
    existing = (
        db.query(Enrollment)
        .filter(Enrollment.class_id == id, Enrollment.user_id == payload.user_id)
        .first()
    )
    if existing:
...
@router.delete(
    "/admin/classes/{id}/enrollments/{user_id}", response_model=ApiEnvelope[dict]
)
def remove_enrollment(
    ...
    row = (
        db.query(Enrollment)
        .filter(Enrollment.class_id == id, Enrollment.user_id == user_id)
        .first()
    )
    if not row:
    ...
    db.commit()
    return ApiEnvelope(
        success=True, data={"deleted": True, "class_id": id, "user_id": user_id}
    )
...
async def list_labs(
    ...
    if not organization_role_at_least(organization, "admin"):
        own_class_ids = select(Class.id).where(
            Class.organization_id == organization.id, Class.instructor_id == _user.id
        )
...
async def create_lab(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="class_id not found or not managed by this instructor",
        ) from exc
    if (
        not db.query(DesktopPool)
        .filter(
            DesktopPool.id == payload.default_pool_id,
...
async def get_lab(
    ...
    row = (
        db.query(Lab)
        .filter(Lab.id == id, Lab.organization_id == organization.id)
        .first()
    )
    if not row:
...
async def patch_lab(
    ...
    if (
        merged.get("starts_at")
        and merged.get("ends_at")
        and merged.get("starts_at") >= merged.get("ends_at")
    ):
        raise HTTPException(status_code=422, detail="starts_at must be before ends_at")
    if (
        payload.default_pool_id is not None
        and not db.query(DesktopPool)
        .filter(
            DesktopPool.id == payload.default_pool_id,
            DesktopPool.organization_id == organization.id,
        )
...
def delete_lab(
    ...
    if db.query(LabRun).filter(LabRun.lab_id == row.id).first():
        raise HTTPException(
            status_code=409, detail="Cannot delete a lab with run history"
        )
    db.delete(row)
    db.commit()
    return ApiEnvelope(success=True, data={"deleted": True, "id": id})
